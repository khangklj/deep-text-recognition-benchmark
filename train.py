...
def train(opt):
    ...
    # print(opt)
    with open(
        f"./saved_models/{opt.exp_name}/opt.txt", "a", encoding="utf-8"
    ) as opt_file:
        opt_log = "------------ Options -------------\n"
        args = vars(opt)
    ...
    # Initialize a DataFrame to store loss and metrics
    ...
    excel_path = f"./saved_models/{opt.exp_name}/training_log.xlsx"
    ...
    while True:
        ...
        if (
            ...
            # for log
            with open(
                f"./saved_models/{opt.exp_name}/log_train.txt", "a", encoding="utf-8"
            ) as log:
                model.eval()
                with torch.no_grad():
                    (
                ...
                # Save loss/accuracy metrics to DataFrame
                            ...
                            [
                                {
                                    ...
                                    "valid_loss": round(float(valid_loss), 5),
                                    "accuracy": round(float(current_accuracy), 4),
                                    "norm_ED": round(float(current_norm_ED), 4),
                ...
                # Save to Excel file each validation interval
                training_log.to_excel(excel_path, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--FT", action="store_true", help="whether to do fine-tuning"
    )  # This flag is already present.
    parser.add_argument(
        "--adam", action="store_true", help="Whether to use adam (default is Adadelta)"
    )
    parser.add_argument(
        "--lr", type=float, default=1.0, help="learning rate, default=1.0 for Adadelta"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="freeze FeatureExtraction parameters",
    )
    parser.add_argument(
        "--freeze_SequenceModeling",
        action="store_true",
        help="freeze SequenceModeling parameters",
    )
    parser.add_argument(
        "--freeze_Prediction", action="store_true", help="freeze Prediction parameters"
    )
